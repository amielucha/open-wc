...
Note that the config will always dedupe `lit-html`, whether you specify it or not. If for some reason you don't want that, you'll have to override the plugin yourself:
...
const isNotNodeResolve = ({ name }) => name !== 'node-resolve';

export default {
  ...config,
  plugins: [resolve({ dedupe: ['my-singleton'] }), ...plugins.filter(isNotNodeResolve)],
};
```
